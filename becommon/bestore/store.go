...
// BeStore defines a concurrent safe in memory key-value data store.
type BeStore[T any] struct {
	data map[string]T
	mux  sync.RWMutex
}

// New creates a new Store[T] instance with a shallow copy of the provided data (if any).
func New[T any](data map[string]T) *BeStore[T] {
	s := &BeStore[T]{}
...
func (s *BeStore[T]) Reset(newData map[string]T) {
	s.mux.Lock()
	defer s.mux.Unlock()
...
func (s *BeStore[T]) Remove(key string) {
	s.mux.Lock()
	defer s.mux.Unlock()
...
func (s *BeStore[T]) Has(key string) bool {
	s.mux.RLock()
	defer s.mux.RUnlock()
...
func (s *BeStore[T]) Get(key string) T {
	s.mux.RLock()
	defer s.mux.RUnlock()
...
func (s *BeStore[T]) GetAll() map[string]T {
	s.mux.RLock()
	defer s.mux.RUnlock()
...
func (s *BeStore[T]) Set(key string, value T) {
	s.mux.Lock()
	defer s.mux.Unlock()
...
func (s *BeStore[T]) SetIfLessThanLimit(key string, value T, maxAllowedElements int) bool {
	s.mux.Lock()
	defer s.mux.Unlock()
